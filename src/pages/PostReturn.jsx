...
import { ArrowLeft, CheckCircle2, UploadCloud, AlertTriangle, AlertOctagon, Clock, XCircle } from 'lucide-react';
...
export default function PostReturn() {
    ...
    const handleConfirm = async () => {
        if ((!fileBlob && condition !== 'cancellation') || !condition || !txData) return;
        setIsSaving(true);
        const toastId = toast.loading(condition === 'cancellation' ? "Processing cancellation..." : "Processing return...");

        try {
            // 1. Upload the post-return photo (skip if cancelled)
            let photoUrl = null;
            if (condition !== 'cancellation' && fileBlob) {
                photoUrl = await uploadTransactionPhoto(fileBlob, id, 'post_return');
            }

            const isGood = condition === 'good' || condition === 'late';
            const finalStatus = condition === 'cancellation' ? 'cancelled' : (isGood ? 'completed' : 'disputed');

            // 2. Update transaction
            await updateTransaction(id, {
                status: finalStatus,
                condition: condition,
                ...(photoUrl && { postReturnPhotoURL: photoUrl }),
                returnedAt: new Date()
            });

            // 3. Recalculate Trust Score for Borrower
            let outcomeType = 'damage';
            if (condition === 'good') outcomeType = 'success';
            else if (condition === 'late') outcomeType = 'late';
            else if (condition === 'cancellation') outcomeType = 'cancellation';

            await recalculateTrustScore(txData.borrowerId, outcomeType);

            if (condition === 'cancellation') {
                toast.success("Transaction cancelled. Trust score penalty applied.", { id: toastId });
            } else if (isGood) {
            ...
            console.error(error);
            toast.error("Failed to process action.", { id: toastId });
        } finally {
            setIsSaving(false);
        }
    };

    const isReady = (photoDataUrl || condition === 'cancellation') && condition !== null && !isSaving;
            ...
            <main className="flex-1 px-4 py-6 pb-32 overflow-y-auto flex flex-col">
                ...
                <div className="mb-auto">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {/* Late */}
                        <button
                            onClick={() => setCondition('late')}
                            className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${condition === 'late'
                                ? 'border-yellow-500 bg-yellow-50'
                                : 'border-gray-100 bg-white hover:border-gray-200'
                                }`}
                        >
                            <Clock size={24} className={condition === 'late' ? 'text-yellow-600' : 'text-gray-300'} />
                            <div className="text-left flex-1">
                                <div className={`font-bold ${condition === 'late' ? 'text-yellow-700' : 'text-dark'}`}>Late Return</div>
                                <div className="text-xs text-gray-500 mt-0.5">Good condition, but returned past due time.</div>
                            </div>
                        </button>
                        ...
                        {/* Cancellation */}
                        <button
                            onClick={() => setCondition('cancellation')}
                            className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${condition === 'cancellation'
                                ? 'border-purple-500 bg-purple-50'
                                : 'border-gray-100 bg-white hover:border-gray-200'
                                }`}
                        >
                            <XCircle size={24} className={condition === 'cancellation' ? 'text-purple-500' : 'text-gray-300'} />
                            <div className="text-left flex-1">
                                <div className={`font-bold ${condition === 'cancellation' ? 'text-purple-700' : 'text-dark'}`}>Cancellation / No-Show</div>
                                <div className="text-xs text-gray-500 mt-0.5">Item was never exchanged. Trust penalty.</div>
                            </div>
                        </button>
                    </div>
                </div>
                ...
                <div className="mt-8">
                    <button
                        ...
                        className={`w-full font-bold text-lg py-4 rounded-pill transition-all ${isReady
                            ? ((condition === 'good' || condition === 'late') ? 'bg-green-500 text-white shadow-lg shadow-green-500/30 hover:scale-[1.02] active:scale-[0.98]' : 'bg-dark text-white shadow-lg')
                            : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                            }`}
                    >
                        {(condition === 'good' || condition === 'late') ? 'Confirm Return & Refund' : condition === 'cancellation' ? 'Confirm Cancellation' : condition ? 'Raise Dispute' : 'Select Condition'}
                    </button>
                </div>
            </main>
        </div>
